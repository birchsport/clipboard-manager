...
**Pin reorder (⌥↑ / ⌥↓) is a sub-state of `.browse`, not a new mode.** `handleBrowse` adds two branches (option-arrow, no cmd/shift) ahead of the existing `switch event.keyCode`. `moveSelectedPin(by:)` walks `entries` (the *visible* list, so the move is predictable when a search filter is active) for the nearest other pinned row in the requested direction and calls `actions.swapPinOrder(a, b)`, which routes to `EntryRepository.swapPinOrder` and swaps the two rows' `pinned_at` timestamps in one write block. No schema change — `pinned_at` is already the sort key, and retention sweeps / archive treat the value as opaque. Selection follows the moved entry across the async refresh via `pendingReselectID`: `moveSelectedPin` sets the id, and `applyFilter` consumes it in `pickSelectedIndex(in:fallback:)` (both the synchronous empty-query and the off-main filtered branches), restoring the cursor onto the row after `entries` is rebuilt. Beeps on an unpinned selection, at the top/bottom of the pinned region, or when no other pinned row is visible.
...
- **`Transforms/`** — a `TextTransform` protocol (`id`, `displayName`, `isApplicable`, `apply`) and a hard-coded `TransformRegistry.all`. Built-ins in `BuiltInTransforms.swift` (JSON / YAML / JWT / timestamp / hashes / number-bases / query-string / Base64 / URL / case / trim / strip / extract). Transforms always produce text that flows through the normal paste path. The three whitespace-trim variants (`whitespace.trim`, `whitespace.rtrim_lines`, `whitespace.trim_lines`) share a private `LineTrim` helper enum in `BuiltInTransforms.swift`. A sibling `lines.trim_borders` ("Trim borders per line") uses its own `LineBorderTrim` helper — same per-line shape, but the edge-character predicate is `Character.isWhitespace || borders.contains(…)`, where `borders` is a `Set<Character>` of ASCII `|` plus the common vertical box-drawing range (U+2502/2503/2551/2506/2507/250A/250B/254E/254F/2575/2577/2579/257B/258F/2595). A second `lines.*` member, `lines.strip_sql_arrows` ("Strip SQL continuation arrows"), strips the `^\s*->\s?` prefix Trino / mysql / Presto / Hive CLIs leave on continuation lines, preserving any further indentation as user content; lines without a leading `->` and mid-line `->` (lambdas, etc.) are untouched. The namespace shift from `whitespace.*` to `lines.*` is deliberate — these transforms do more than whitespace, and future per-line cleanups slot into the same family. All five `isApplicable` only when applying would change the payload.
